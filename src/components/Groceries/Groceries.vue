<template>
	<Page ref="page" :class="pageClasses" actionBarHidden="true" backgroundSpanUnderStatusBar="true">
		<GridLayout ref='container' rows="auto, auto, *" class="list-page" @loaded="load()">
			...
			<GridLayout row="1" columns="auto, *, auto" class="add-bar" :backgroundColor="isShowingRecent ? '#BBC169' : '#1E9AFF'">
				...
				<TextField ref='groceryTextField' col="1" v-model="grocery" @loaded="handleAndroidFocus()" :hint="isAndroid ? 'ADD A GROCERY' : 'Add a grocery'" returnKeyType="done" v-else @returnPress="add('textfield')"></TextField>
			...
			<GroceryList row="2" :listLoaded="listLoaded" :showDeleted="isShowingRecent" :items="items"></GroceryList>
